...
<body>
                                    ...
                                    <c:forEach items="${shipments}" var="shipment">
                                        <c:if test="${shipment.id eq shipmentId}">
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="dropdown">
                                                        <button class="btn btn-outline-primary btn-sm dropdown-toggle" type="button" id="driversDropdown${shipment.id}" data-bs-toggle="dropdown" aria-expanded="false">
                                                            <span class="badge bg-primary">${interests.size()} Drivers</span>
                                                        </button>
                                                        <ul class="" aria-labelledby="driversDropdown${shipment.id}" style="min-width: 250px;">
                                                            <c:forEach items="${interests}" var="interest">
                                                                <li class="dropdown-item interest-item" id="interest-${interest.id}">
                                                                    <div class="d-flex justify-content-between align-items-center">
                                                                        <div class="me-2">
                                                                            <strong>${interest.driver.name}</strong><br>
                                                                            <small class="text-muted">
                                                                                <!-- Role-based email visibility -->
                                                                                <i class="fas fa-envelope me-1"></i>
                                                                                <c:set var="email" value="${interest.driver.email}" />
                                                                                <c:set var="username" value="${fn:substringBefore(email, '@')}" />
                                                                                <c:set var="domain" value="${fn:substringAfter(email, '@')}" />
                                                                                <c:choose>
                                                                                    <c:when test="${fn:length(username) <= 3}">
                                                                                        ${username}@${domain}
                                                                                    </c:when>
                                                                                    <c:otherwise>
                                                                                        ${fn:substring(username, 0, 3)}...@${domain}
                                                                                    </c:otherwise>
                                                                                </c:choose>
                                                                            </small>
                                                                        </div>
                                                                        <c:choose>
                                                                            <c:when test="${interest.status eq 'PENDING'}">
                                                                                <div class="btn-group btn-group-sm" role="group">
                                                                                    <button class="btn btn-success accept-driver-btn" 
                                                                                            data-interest-id="${interest.id}"
                                                                                            title="Accept Driver">
                                                                                        <i class="fas fa-check"></i>
                                                                                    </button>
                                                                                    <button class="btn btn-danger reject-driver-btn"
                                                                                            data-interest-id="${interest.id}"
                                                                                            title="Reject Driver">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.accept-driver-btn', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            ...
                            btnGroup.replaceWith('<span class="badge bg-success interest-status">Accepted</span>');
                            ...
                            btnGroup.find('.accept-driver-btn').html('<i class="fas fa-check"></i>');
            ...
            $(document).on('click', '.reject-driver-btn', function() {
                const interestId = $(this).data('interest-id');
                const btnGroup = $(this).closest('.btn-group');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            ...
                            btnGroup.replaceWith('<span class="badge bg-danger interest-status">Rejected</span>');
                            ...
                            // Re-enable buttons on error
                            btnGroup.find('button').prop('disabled', false);
                            btnGroup.find('.reject-driver-btn').html('<i class="fas fa-times"></i>');
                            showNotification('Error', response.message || 'Something went wrong');
                        }
                    },
                    error: function() {
